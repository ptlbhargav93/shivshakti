  ...
  if (submit_form == false) {    
    ...
    if (window['finalFiles'][formID][uploadFileEleId] != undefined) {
      if (Object.keys(window['finalFiles'][formID][uploadFileEleId]).length > 0) {
        ...
        var form = $("#" + formID);
        if ($(form).find("#form_submit, #send_invoice_via_email_form_submit").length > 0) {
          if ($(form).find("#object_type"+uploadFileEleId).val() == 'temp_folder_send_invoice') {
            formData.append('id', $(form).find('#send_bill_id').val());
            var send_bill_id = $(form).find("#send_bill_id").val();
            var DataUrl = "/send_bills/"+send_bill_id+"/create_temp_folder";
            var type = "PUT";
            if ($(form).find("#send_bill_subject").val() == "" || $(form).find("#send_bill_body").val() == ""){
              return;
            }
          }
          else{
      ...
      $.each(submitBtns, function(index, submitBtn){
        $(submitBtn).on('click', function(event){
          if ($(form).find("#upload_photos").length > 0) {
            if (window['finalPhotos'] != undefined) {
              if (Object.keys(window['finalPhotos']).length > 0){
